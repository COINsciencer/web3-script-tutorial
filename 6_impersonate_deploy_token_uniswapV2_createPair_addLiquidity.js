...
async function deploy(abi, bytecode, signer) {
    const factory = new hardhat.ethers.ContractFactory(abi, bytecode, signer);
...
async function getReserves(pairContract) {
    ...
    console.log(`reserves: ${hardhat.ethers.formatEther(reserves[0])}, ${hardhat.ethers.formatUnits(reserves[1], 18)}`)
}
...
async function main() {
	...
	const PIGContract = await deploy(PIGABI, bytecode, signer)

	// mint PIG
	const mintAmount = hardhat.ethers.parseUnits('1000000', 18)
	...
	console.log(`recipient PIG balance: ${hardhat.ethers.formatUnits(PIGBalance, 18)}`)
    ...
    // approve
    const txApprovePIG = await PIGContract.connect(signer).approve(routerAddress, hardhat.ethers.MaxUint256);
    await txApprovePIG.wait();
    const txApproveWETH = await WETHContract.connect(signer).approve(routerAddress, hardhat.ethers.MaxUint256);
    ...
    const txAddLiquidity = await routerContract.connect(signer).addLiquidity(
        PIGContract.target,
        WETH_ADDRESS,
        hardhat.ethers.parseUnits('1000000', 18),
